...
class Blender:
    ...
    def render_image(self, *, params: RenderParams, output_path: Path):
        """
        Renders the current scene with the given parameters. All the fields in
        `params` are assumed to be validated beforehand except for `input_path`
        and `output_path`.
        """
...
class ServerApp(flask.Flask):
    ...
    def _render_endpoint(self):
        ...
        try:
            ...
            return flask.send_file(buffer, mimetype=f"image/png")
