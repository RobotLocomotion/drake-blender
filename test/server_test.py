...
class ServerTest(unittest.TestCase):
    ...
    def test_gltf_render(self):
        ...
        """
        TestCase = namedtuple(
            "TestCase", ["image_type", "scene_file", "dtype", "threshold"]
        )
        test_cases = [
            TestCase("color", "scene", np.uint8, COLOR_PIXEL_THRESHOLD),
            TestCase("label", "scene_label", np.uint8, LABEL_PIXEL_THRESHOLD),
            TestCase("depth", "scene", float, DEPTH_PIXEL_THRESHOLD),
        ]
        for test_case in test_cases:
            with self.subTest(image_type=test_case.image_type):
                self.check_gltf_render(**test_case._asdict())

    def check_gltf_render(self, *, image_type, scene_file, dtype, threshold):
        """The implementation of test_gltf_render on a specific image_type.
        """
        with open(f"test/{scene_file}.gltf", "rb") as scene:
